// Curtain Fabric Matrix - loaded from server
let CURTAIN_FABRIC_MATRIX = null;
...
class CurtainPricing {
    ...
    async loadFabricMatrix() {
        try {
            const response = await fetch('/PriceList/GetCurtainFabricMatrix');
            const data = await response.json();

            if (data.fabricUsage) {
                // Transform the data to match the expected format
                CURTAIN_FABRIC_MATRIX = {
    ...
    calculatePrices() {
        ...
        if (!this.matrixLoaded || !CURTAIN_FABRIC_MATRIX) {
